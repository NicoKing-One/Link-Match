...
- 当前阶段：`demo1.1.3首页资源提交` 已将新版首页三主题 UI 资源包提交到 `src/assets/UI-Home/`，包含水果森林、糖果花园、果冻城堡三套首页背景、章节 tab、关卡状态底图、标题底图、开始按钮，以及通用资源卡、顶部按钮、翻页箭头、锁、个人/设置 icon、资源清单和总览图；当前仅提交资源，首页 H5 结构尚未切换到这批资源。

## 2. 当前进度

- 2026-06-16 首页资源提交：`src/assets/UI-Home/` 已纳入仓库，资源来自三张已确认的新版首页设计稿，包含通用资源 `top-button-bg.png`、`icon-profile.png`、`icon-settings.png`、`resource-card-bg.png`、`arrow-button-bg.png`、`icon-arrow-left.png`、`icon-arrow-right.png`、`icon-lock.png`；水果森林、糖果花园、果冻城堡三主题分别包含完整背景、选中/未选中 tab、章节标题底图、当前/已完成/未开始关卡底图、开始按钮底图；另包含 `asset-manifest.md` 和 `_contact-sheet.png` 便于后续接入核对。当前本轮只提交资源文件和交接文档，不改首页运行逻辑。
...
当前已完成 H5 单机 demo 的核心玩法、体力系统、道具占位、结算流程、移动端游戏 UI、第一版章节闯关首页布局、`demo1.0.6-UI布局更新`、`demo1.0.7--金币系统优化`、`demo1.0.8--首页二级页面开发+UI`、`demo1.0.9个人中心页面UI完成`、`demo1.1.1个人中心页面三星关卡bug修复`、`demo1.1.2设置页面UI优化`、`demo1.1.3首页资源提交`。首页 UI 方向已执行还原清理：旧三主题高保真稿、首页背景图、对齐稿、取用规则说明、导出模板/脚本和旧首页截图均已删除；新版三主题首页完整 UI 设计稿已重新确定并保存，且首页资源包已提交到 `src/assets/UI-Home/`。
...
- 首页和章节关卡：
  ...
  - 新版首页资源包已提交到 `src/assets/UI-Home/`，按通用资源和三主题资源分类，资源清单见 `src/assets/UI-Home/asset-manifest.md`；当前尚未接入 H5 首页渲染。
...
- `scripts/smoke-browser.mjs`：浏览器自动化验收脚本，使用固定 `boardSeed`，包含首页地图、首页背景无图片断言、个人中心三星关卡一致性断言、设置页 UI 精修断言、金币兑换页布局断言、移动端截图和主流程 UI 断言。
...
- 单元测试：26/26 通过。
- 构建：成功输出到 `dist/`。
- 浏览器 smoke：使用固定 seed 通过，覆盖首页地图、首页背景无图片、个人中心/设置/金币兑换三个二级入口、二级页禁止整页设计图铺底、个人中心 UI 精修断言、设置页 UI 精修断言、金币兑换页统一布局断言、个人中心三星关卡数与首页 `bestStars` 星级节点一致性、游戏页、弹框、结果页、体力流程、首通金币、重复通关无金币文案和 0 道具不呼吸状态。
- 首页旧设计稿与导出脚本已删除，本条验证记录不再作为当前首页设计依据。
- 新版首页三主题设计稿已做本地文件检查：三张图片均存在，已移除截图外侧黑色阴影并保存为带透明通道的 PNG。
- 首页资源包已做本地文件检查：`src/assets/UI-Home/` 下通用资源、三主题资源、`asset-manifest.md` 和 `_contact-sheet.png` 均存在。
...
- 本轮提交备注：`demo1.1.3首页资源提交`。
- 本轮提交内容：提交 `src/assets/UI-Home/` 首页资源包，包含通用首页组件资源、水果森林/糖果花园/果冻城堡三主题首页切图、资源清单和总览图；同步更新交接文档。当前不改首页运行逻辑。
...
1. 使用 `src/assets/UI-Home/` 已提交资源接入首页 UI，保持文字和动态数值在 HTML/CSS 层，不把中文文案烘焙进切图。
2. 首页实现时先固定三主题共同布局：个人/设置/资源卡/章节 tab/左右箭头/标题条/关卡路线/底部按钮同尺寸同坐标。
3. 接入 H5 首页时以 `home-ui-design-v2-*.png` 三张新版设计稿和 `src/assets/UI-Home/asset-manifest.md` 为核对依据；截图式设计稿只作为视觉参考，不整张贴进游戏。
4. 首页 UI 落地后补 smoke 断言：检查三主题切换、资源卡间距、当前主题高亮、锁定章节、关卡星级底座、底部按钮位置，以及 `src/assets/UI-Home/` 资源实际渲染。
5. 二级页后续替换正式图片/icon 时，保持当前 HTML/CSS 结构不变，只替换 `src/assets/UI-ICON/*-page/` 下的独立资源；个人中心当前版已按 demo1.1.1 收口，设置页当前版已按 demo1.1.2 收口，后续优先避免破坏已写入 smoke 的视觉和数据一致性约束。
6. 接入金币兑换实际逻辑：定义体力、提示、洗牌的兑换比例、金币扣除、资源增加和确认/失败提示。
7. 接入设置实际逻辑：音乐/音效/振动持久化；若后续重新加入清除本地进度，需要先确认新的入口位置和确认弹框样式。
