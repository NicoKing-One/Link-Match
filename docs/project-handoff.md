...
- 当前阶段：`1.1.3-增加兑换商城` 已按确认预览接入 3x3 头衔商城效果，两页共 18 个头衔商品，支持左右翻页、金币余额判断、兑换成功/金币不足弹框；当前只做头衔商城 UI 和交互反馈，不扣金币、不持久化已拥有/已佩戴状态。新版首页三主题 UI 资源包已提交到 `src/assets/UI-Home/`，首页 H5 结构尚未切换到这批资源。
...
- 单元测试：26/26 通过。
- 构建：成功输出到 `dist/`。
- 浏览器 smoke：使用固定 seed 通过，覆盖首页地图、首页背景无图片、个人中心/设置/金币兑换三个二级入口、二级页禁止整页设计图铺底、个人中心 UI 精修断言、设置页 UI 精修断言、金币兑换页统一布局断言、3x3 头衔商城两页翻页、兑换成功弹框、金币不足弹框、旧体力/提示/洗牌兑换项移除断言、个人中心三星关卡数与首页 `bestStars` 星级节点一致性、游戏页、弹框、结果页、体力流程、首通金币、重复通关无金币文案和 0 道具不呼吸状态。
...
- 本轮提交备注：`1.1.3-增加兑换商城`。
- 本轮提交内容：接入金币兑换页 3x3 头衔商城，两页共 18 个头衔商品，支持左右翻页、金币余额判断、兑换成功/金币不足弹框；同步优化金币栏文案、尺寸和金币 icon 裁切；更新 smoke 回归断言和交接文档。
...
6. 完善头衔系统闭环：定义头衔拥有状态、已佩戴状态、金币扣除与持久化规则，并决定重复点击已拥有头衔时显示“已拥有”还是切换佩戴。
