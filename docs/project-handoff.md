...
- 当前阶段：`1.2.5文件优化` 已完成：所有运行时图片已统一平铺在 `src/assets/image/` 下；代码、样式、脚本和文档引用已同步到新路径；本版本不上线的兑换商城页面、旧商城资源、旧设计稿 PNG、旧预览图、重复图片和未引用图片已清理。上一轮 `1.2.4修复bug` 已提交并同步远程，后台计时修复、失败复活广告等能力保持可用。
...
- `src/game.js`：游戏状态、首页地图渲染、二级页面切换、个人中心数据同步、设置开关前端状态、敬请期待弹框、关卡标题同步、读档/新档默认值、道具和结算逻辑。
...
- `scripts/smoke-browser.mjs`：浏览器自动化验收脚本，使用固定 `boardSeed`，包含首页地图、三主题图片连接线、当前关卡居中、游戏/暂停返回首页后仍定位当前关卡、首页无清空数据入口、首页资源卡精简、首页兑换入口“敬请期待”、顶部金币卡不可打开商城、章节 tab 移除、章节标题栏/首页箭头资源渲染、个人中心三星关卡一致性断言、设置页 UI 精修断言、双倍金币广告奖励断言、失败复活广告完成/未完成断言、移动端截图和主流程 UI 断言。

## 4. 最近验证

最近一轮（2026-06-25，资源清理后）已执行：
...
- 完整 smoke：`scripts/smoke-browser.mjs` 通过，输出 `Smoke passed: 42 tiles rendered at http://127.0.0.1:65368`，覆盖首页地图、水果森林/糖果花园/水晶城堡图片连接线与新节点结构、底部首关贴边、数字牌居中、首页独立兑换入口“敬请期待”、顶部金币卡展示态、二级页、主流程、真实时间差倒计时失败、双倍金币广告奖励、失败复活广告完成/未完成、结算弹框、通关返回首页和资源渲染。
...
- `dist/` 和 `output/` 只在构建、资源处理或 smoke 验证时临时生成；本轮资源清理后已删除，不提交到仓库。

## 5. 当前 Git 状态

- 当前分支：`main`
- 远程仓库：`https://github.com/NicoKing-One/Link-Match.git`
- 本轮收口内容：`1.2.5文件优化`，包含运行时图片资源平铺到 `src/assets/image/`、旧图片/设计稿/输出产物清理、兑换商城运行时代码移除，以及相关引用和 smoke 断言同步。
- 本轮提交备注：`1.2.5文件优化`。
