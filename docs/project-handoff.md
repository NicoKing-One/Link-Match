...
- 当前阶段：`1.2.6bug修复` 已完成：围绕正式测试问题汇总修复复活二次失败按钮、返回首页章节同步、窄屏首页比例适配、结果页星级 aria-label 乱码和 favicon 404；相关回归断言已写入 smoke，并通过单测、资源检查、构建和完整浏览器 smoke。上一轮 `1.2.5文件优化` 已完成，运行时图片仍统一平铺在 `src/assets/image/` 下。
...
- 2026-06-25 `1.2.5文件优化`：将运行时图片资源统一迁移并平铺到 `src/assets/image/` 根目录，不再保留图片分类子目录；`src/index.html`、`src/styles.css`、`src/game.js`、资源检查脚本、构建/smoke 脚本和文档引用均已改为 `assets/image/文件名`。本版本不上线的兑换商城已从运行时代码中移除：删除旧 `#exchangeScreen` DOM、头衔商城 JS 渲染逻辑、商城 CSS、商城 smoke 断言和商城独立资源；首页左侧兑换入口继续保留，只弹“敬请期待”。同时删除旧设计稿 PNG、旧视觉参考图、contact sheet、旧 tab/箭头图、重复根目录 UI 图、已废弃清档/返回首页资源、`tile-sheet.png`/`tile-preview.png` 等未引用资源。清理后除 `.git` 内部历史对象外，项目图片只保留在 `src/assets/image/` 根目录下。已执行单测、资源检查、构建和完整 smoke，均通过。
...
- 首页二级页面：
  ...
  - 首页金币资源框 `#coinExchangeButton` 只展示金币余额，不再承担点击入口；首页地图左侧 `#homeExchangeButton` 本版本只弹出“敬请期待”，不进入兑换商城页面。
  ...
  - 金币兑换商城本版本不上线，旧 `#exchangeScreen`、头衔商城 DOM/JS/CSS、商城资源和商城 smoke 断言已清理；保留首页兑换入口作为“敬请期待”提示。
  - 个人中心和设置页均只保留左上返回按钮，底部“返回首页”按钮已移除。
  - `scripts/smoke-browser.mjs` 已新增断言：二级页必须由布局根节点渲染，若重新使用整页设计图会失败；个人中心覆盖无滚动、返回按钮尺寸、标题文字位置、6 个数据框居中/无边框、统计字号、`0关` 后缀、三星关卡与首页星级数据一致、玩家信息框 `margin-top: 50px`、底部返回按钮移除等回归检查；设置页覆盖 3 个设置行、清除进度框移除、行背景/卡片无额外白边、行内元素上下居中、设置 icon 尺寸和左边距、设置行列宽/高度、开关 padding/圆点尺寸与位置、面板 padding 等回归检查；首页兑换入口覆盖只弹出“敬请期待”、不进入商城页。
...
- 玩法和系统：
  ...
  - 新档默认基线为：金币 0、星星 0、无通关记录、第 1 关未通关、体力 50/50、广告领取次数 0、提示 0、洗牌 0。
...
- `src/game.js`：游戏状态、首页地图渲染、首页道路随容器宽度缩放、二级页面切换、个人中心数据同步、设置开关前端状态、敬请期待弹框、关卡标题同步、读档/新档默认值、道具和结算逻辑。
...
- `scripts/smoke-browser.mjs`：浏览器自动化验收脚本，使用固定 `boardSeed`，包含 favicon 加载、首页地图、320/360/390 视口比例缩放、三主题图片连接线、当前关卡居中、浏览锁定章节后返回首页同步当前章节、游戏/暂停返回首页后仍定位当前关卡、首页无清空数据入口、首页资源卡精简、首页兑换入口“敬请期待”、顶部金币卡不可打开商城、章节 tab 移除、章节标题栏/首页箭头资源渲染、个人中心三星关卡一致性断言、设置页 UI 精修断言、双倍金币广告奖励断言、失败复活广告完成/未完成/二次失败隐藏复活按钮断言、移动端截图和主流程 UI 断言。

## 4. 最近验证

最近一轮（2026-06-25，`1.2.6bug修复` 后）已执行：
...
- 单测：`tests/*.test.mjs` 通过，27 个测试全部通过，覆盖引擎固定随机、关卡难度档、进度和金币规则。
...
- 构建：`scripts/build.mjs` 验证通过；`dist/` 属临时构建产物，验证后已清理，不作为仓库资源保留。
- 完整 smoke：`scripts/smoke-browser.mjs` 通过，输出 `Smoke passed: 42 tiles rendered at http://127.0.0.1:51433`，覆盖 favicon 加载、首页地图、320/360 宽度比例缩放、水果森林/糖果花园/水晶城堡图片连接线与新节点结构、底部首关贴边、数字牌居中、首页独立兑换入口“敬请期待”、顶部金币卡展示态、浏览锁定章节后返回首页同步当前章节、二级页、主流程、真实时间差倒计时失败、双倍金币广告奖励、失败复活广告完成/未完成/二次失败隐藏复活按钮、结算弹框、通关返回首页和资源渲染。
...
- 高优先级：复活成功后再次失败不再显示“复活”按钮，避免同一局二次复活入口误导。
- 高优先级：浏览其它章节后通过“开始闯关”进入当前进度关卡，再从游戏页返回首页，会同步回当前进度章节并定位当前关卡。
- 中优先级：首页 320/360 宽度下关键首页 UI 跟随容器比例缩放，章节标题、资源卡、关卡节点和道路尺寸纳入 smoke 回归。
- 中优先级：结果页星级 aria-label 乱码已修正。
- 低优先级：favicon 404 已通过新增 `src/favicon.ico` 和 `.ico` MIME 修复。
...
- 本轮收口内容：`1.2.6bug修复`，包含复活二次失败按钮隐藏、返回首页章节同步、首页窄屏比例缩放、结果页 aria-label 修正、favicon 404 修复、开发/smoke 静态服务 `.ico` MIME、bug 修复文档和 smoke 回归断言同步。
- 本轮提交备注：`1.2.6bug修复`。
