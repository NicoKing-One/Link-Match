...
- 当前阶段：`1.2.5首页优化` 已完成：首页资源框按 40% icon / 60% 文案区域重排，体力上限调整为 60 并移除自然恢复倒计时，个人中心昵称改为新档随机生成并持久化。上一轮 `1.2.7体力显示与恢复逻辑调整` 已完成。
...
- `scripts/smoke-browser.mjs`：浏览器自动化验收脚本，使用固定 `boardSeed`，包含 favicon 加载、首页地图、320/360/390 视口比例缩放、首页资源框 40%/60% 几何布局、三主题图片连接线、当前关卡居中、浏览锁定章节后返回首页同步当前章节、游戏/暂停返回首页后仍定位当前关卡、首页无清空数据入口、首页兑换入口“敬请期待”、顶部金币卡不可打开商城、章节 tab 移除、章节标题栏/首页箭头资源渲染、个人中心昵称和三星关卡一致性断言、设置页 UI 精修断言、双倍金币广告奖励断言、失败复活广告完成/未完成/二次失败隐藏复活按钮断言、移动端截图和主流程 UI 断言。

## 4. 最近验证

最近一轮（2026-06-26，`1.2.5首页优化` 收口时）已执行：
...
- 单测：`tests/*.test.mjs` 通过，27 个测试全部通过，覆盖引擎固定随机、关卡难度档、进度/金币规则、随机玩家昵称和体力每日重置规则。
...
- 完整 smoke：`scripts/smoke-browser.mjs` 通过，覆盖 favicon 加载、首页地图、首页资源框 40%/60% 几何布局、320/360 宽度比例缩放、水果森林/糖果花园/水晶城堡图片连接线与新节点结构、底部首关贴边、数字牌居中、首页独立兑换入口“敬请期待”、顶部金币卡展示态、浏览锁定章节后返回首页同步当前章节、二级页、个人中心昵称、主流程、真实时间差倒计时失败、双倍金币广告奖励、失败复活广告完成/未完成/二次失败隐藏复活按钮、结算弹框、通关返回首页和资源渲染。
...
- 本轮收口内容：`1.2.5首页优化`，包含首页资源框布局微调、体力 60 上限与每日重置规则、个人中心随机昵称持久化、相关单测与 smoke 回归断言同步。
- 本轮提交备注：`1.2.5首页优化`。
