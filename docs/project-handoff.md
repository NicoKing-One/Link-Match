...
- 当前阶段：`1.2.7 UI优化` 提交收口：按本轮备注收口当前工作区累计改动，已完成提交前验证、本地提交和远程 `main` 推送。
...
最近一轮（2026-06-29，`1.2.7 UI优化` 提交前收口时）已执行：
...
- 单测：`tests/*.test.mjs` 通过，46 个测试全部通过，覆盖引擎固定随机、统一棋盘布局下的难度时间档、进度/金币规则、随机玩家昵称、体力每日重置规则、版本化清档、当前真机旧档体力一次性补满、体力弹窗文案、首页主题资源预加载、真机预览地址和端口回退，以及设置页音频开关会话态、刷新重置、统一按钮音效和当前背景音乐/失败音效口径。
- 资源检查：`scripts/check-home-road-node-assets.py` 通过，9 张首页关卡节点资源保持 `1024px x 1024px` 画布和 `912px` 可见高度，糖果/水晶连接线无透明边缘留白。
- 构建：`scripts/build.mjs` 验证通过；`dist/` 属临时构建产物，不作为仓库资源保留。
- 完整 smoke：`scripts/smoke-browser.mjs` 通过，覆盖 favicon 加载、首页地图、首页资源框 40%/60% 几何布局、金币文本位置、320/360 宽度比例缩放、水果森林/糖果花园/水晶城堡图片连接线与新节点结构、底部首关贴边、数字牌居中、未开放功能统一文案、顶部金币卡展示态、浏览锁定章节后返回首页同步当前章节、二级页、个人中心昵称、设置页开关本次会话关闭且刷新后恢复开启、刷新后首次触摸可解锁背景音乐、打开小游戏首页即触发原创 WebAudio 背景音乐、三档关卡统一棋盘布局、棋子按压反馈、提示/洗牌广告奖励、洗牌动画中途换盘且动画结束后棋盘不再变化、主流程、真实时间差倒计时失败、双倍金币广告奖励、失败复活广告完成/未完成/二次失败隐藏复活按钮、结算弹框、通关返回首页和资源渲染。本轮 smoke 输出 `Smoke passed: 42 tiles rendered`。
- 当前关卡定位：smoke 已模拟进度到第 6 关，验证首页初次进入、游戏页返回首页和暂停弹框返回首页后，第 06 关会居中显示在关卡地图可视区域。
- 游戏页视觉：smoke 临时截图已确认新高饱和果冻花园背景渲染正常，`.board-wrap` 保留浅色实线边框且无背景/阴影覆盖；easy/normal/hard 三档当前保持相同棋盘可视尺寸和 4px 格间距，底部工具栏不再被高行数棋盘挤压。
...
- 首页资源框：体力、金币、星星三张资源卡均按 40% icon / 60% 文案区域渲染，icon 为 `8cqw` 并右贴 icon 区，文案为 `4cqw` 并带 `margin-left: 1cqw`，金币数值额外右移 `3.1282cqw`。
- 棋盘和道具：三档关卡统一使用 easy 棋盘布局；棋子点击有按压反馈；洗牌道具会先四散旧棋子，中途替换为新棋盘，再让新棋子回拢，动画结束后棋盘不再突变。
- 体力系统：新档与每日重置均回到 `60/60`；当前真机旧档体力为 0 时只补满一次体力，不清空关卡进度；经过 3 分钟或更久不会自然恢复。
- 真机预览：开发服务在 `4173` 被占用时会自动尝试后续端口，继续打印 Local 和 Network 访问地址。
...
- 提交前基线：`a471344 1.2.6bug修复`，当前 `main...origin/main` 无领先/落后提交。
- 工作区状态：本轮改动已按备注 `1.2.7 UI优化` 完成本地提交并推送到远程 `main`。
- 本轮提交文件：`docs/project-handoff.md`、`scripts/dev-server.mjs`、`scripts/smoke-browser.mjs`、`src/engine.js`、`src/game.js`、`src/levels.js`、`src/storage-reset.js`、`src/styles.css`、`tests/dev-server.test.mjs`、`tests/engine.test.mjs`、`tests/progression.test.mjs`、`tests/storage-reset.test.mjs`。
- 本轮收口内容：按备注 `1.2.7 UI优化` 收口当前工作区累计改动，包含统一关卡棋盘布局、棋子按压反馈、洗牌动画体验、道具广告奖励、真机预览端口回退、旧档体力一次性补满和配套回归断言同步。
- 本轮提交备注：`1.2.7 UI优化`。
