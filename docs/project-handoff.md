...
- 当前阶段：`1.2.2游戏优化` 本地实现完成：首页金币资源卡恢复为纯展示余额，兑换商城改由首页左侧独立入口进入；首次通关结果页新增“双倍金币”广告奖励按钮，支持广告完成后追加一次首通金币奖励、未看完不发放、重复点击不重复发放；结算页已移除得分/最佳统计块并补充结果页内提示条；设置开关、道具图标和道具数量角标完成同步细化。
...
- `scripts/smoke-browser.mjs`：浏览器自动化验收脚本，使用固定 `boardSeed`，包含首页地图、三主题图片连接线、当前关卡居中、游戏/暂停返回首页后仍定位当前关卡、首页清空数据按钮、首页资源卡精简、独立兑换商城入口、顶部金币卡不可打开商城、章节 tab 移除、章节标题栏/兑换页箭头资源渲染、个人中心三星关卡一致性断言、设置页 UI 精修断言、金币兑换页布局断言、头衔商城翻页和兑换结果弹框断言、双倍金币广告奖励断言、旧体力/提示/洗牌兑换项移除断言、移动端截图和主流程 UI 断言。
...
- 本轮收口内容：`1.2.2游戏优化`，包含首页兑换商城独立入口、顶部金币卡展示态、首次通关双倍金币广告奖励、结果页提示条、结果页得分/最佳统计块移除、设置开关过渡、道具图标替换和道具数量角标调整。
- 本轮提交备注：`1.2.2游戏优化`。
