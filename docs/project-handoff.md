...
- 当前阶段：`1.2.8优化` 提交收口：按本轮备注收口当前工作区累计改动，已完成提交前验证、本地提交和远程 `main` 推送。
...
最近一轮（2026-07-01，`1.2.8优化` 提交前收口时）执行：
...
- 单测：`tests/*.test.mjs` 通过，48 个测试全部通过，覆盖引擎固定随机、移除旧 demo `LEVELS` 导出、统一棋盘布局下的难度时间档、进度/金币规则、随机玩家昵称、体力每日重置规则、版本化清档、当前真机旧档体力一次性补满、体力弹窗文案、首页主题资源预加载、真机预览地址和端口回退，以及设置页音频开关会话态、刷新重置、统一按钮音效、当前背景音乐/失败音效口径和首次手势前 suspended WebAudio 延迟调度。
- 资源检查：`scripts/check-home-road-node-assets.py` 通过，9 张首页关卡节点资源保持 `1024px x 1024px` 画布和 `912px` 可见高度，糖果/水晶连接线无透明边缘留白，糖果完成态中心色相与当前态粉色调保持一致。
- 构建：`scripts/build.mjs` 验证通过；`dist/` 属临时构建产物，不作为仓库资源保留。
- 完整 smoke：`scripts/smoke-browser.mjs` 通过，覆盖 favicon 新路径、首页地图、首页资源框 40%/60% 几何布局、金币文本位置、320/360 宽度比例缩放、水果森林/糖果花园/水晶城堡图片连接线与新节点结构、主题章节箭头资源、锁关数量、底部首关贴边、数字牌居中、游戏页主题背景、未开放功能统一文案、顶部金币卡展示态、浏览锁定章节后返回首页同步当前章节、二级页、个人中心昵称、设置页开关本次会话关闭且刷新后恢复开启、刷新后首次触摸可解锁背景音乐、打开小游戏首页即触发原创 WebAudio 背景音乐、三档关卡统一棋盘布局、棋子按压反馈、提示/洗牌广告奖励、洗牌动画中途换盘且动画结束后棋盘不再变化、主流程、真实时间差倒计时失败、双倍金币广告奖励、失败复活广告完成/未完成/二次失败隐藏复活按钮、结算弹框、通关返回首页和资源渲染。本轮 smoke 输出 `Smoke passed: 42 tiles rendered`。
...
- 主题资源：糖果/水晶首页背景替换为 premium 完整背景，章节箭头按主题切换资源，游戏页背景随当前关卡章节切换；旧糖果/水晶完整背景、旧单一游戏页果冻背景和根目录 favicon 已迁移或删除。
- 文档与维护：README/PRD 已改为三档统一 `6列 x 7行` 的当前实现口径；`src/engine.js` 不再导出旧 demo `LEVELS`，避免和 `src/levels.js` 的正式 90 关配置混淆；首页首次加载时减少 suspended WebAudio 上下文在用户手势前的 BGM 调度。
...
- 提交前基线：`c930311 1.2.7 UI优化`，当前 `main...origin/main` 无领先/落后提交。
- 工作区状态：本轮改动已按备注 `1.2.8优化` 完成本地提交并推送到远程 `main`。
- 本轮提交文件：README/PRD/design/handoff/测试报告同步，主题背景和章节箭头资源变更，favicon 路径迁移，首页资源预加载、主题背景、锁关和 smoke 断言更新，版本化清档标记更新，`src/engine.js` 旧 demo `LEVELS` 清理，WebAudio 首次手势前调度优化，以及对应单测/资源检查脚本更新。
- 本轮收口内容：按备注 `1.2.8优化` 收口当前工作区累计改动，包含文档口径同步、三主题背景与箭头资源优化、游戏页主题背景切换、新档锁关口径、旧 demo 常量清理、首次手势前 WebAudio 调度减少和配套回归断言同步。
- 本轮提交备注：`1.2.8优化`。
