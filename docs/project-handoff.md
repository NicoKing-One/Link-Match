...
- 当前阶段：`1.1.5首页UI优化` 已完成首页资源重切与布局收口：资源卡只保留 icon + 数值（体力保留刷新倒计时），章节三 tab 已移除并由中间章节关卡栏承接，左右切换按钮改用金币兑换页翻页按钮资源，底部开始按钮按原图比例铺满容器并保留文字下压 padding。金币兑换逻辑仍保持展示和反馈版本：不扣金币、不持久化已拥有/已佩戴状态。
...
当前已完成 H5 单机 demo 的核心玩法、体力系统、道具占位、结算流程、移动端游戏 UI、第一版章节闯关首页布局、`demo1.0.6-UI布局更新`、`demo1.0.7--金币系统优化`、`demo1.0.8--首页二级页面开发+UI`、`demo1.0.9个人中心页面UI完成`、`demo1.1.1个人中心页面三星关卡bug修复`、`demo1.1.2设置页面UI优化`、`demo1.1.3首页资源提交`、`1.1.4-首页UI替换`、`1.1.4兑换页面优化完成`、`1.1.5首页UI优化`。首页 UI 方向已执行还原清理：旧三主题高保真稿、首页背景图、对齐稿、取用规则说明、导出模板/脚本和旧首页截图均已删除；新版三主题首页完整 UI 设计稿已重新确定并保存，且首页资源包已接入 H5 首页渲染。
...
- 本轮提交备注：`1.1.5首页UI优化`。
- 本轮提交内容：优化首页 UI 资源与布局，重切首页组件透明余边和主题色；资源卡改为 icon + 数值布局，章节 tab 移除并改为左右箭头 + 章节标题栏，左右箭头复用兑换页翻页按钮资源，底部开始按钮按原图比例铺满容器；同步 `scripts/smoke-browser.mjs` 首页回归断言和交接文档。
