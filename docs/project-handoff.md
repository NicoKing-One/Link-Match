...
- 当前阶段：`demo1.0.8--首页二级页面开发+UI` 已完成首页三处二级入口开发：个人中心、设置页、金币兑换页均已接入真实 HTML/CSS 页面布局，不再把整页设计稿铺满当页面使用；后续可直接替换独立图片/icon 并接入功能逻辑。
...
当前已完成 H5 单机 demo 的核心玩法、体力系统、道具占位、结算流程、移动端游戏 UI、第一版章节闯关首页布局、`demo1.0.6-UI布局更新`、`demo1.0.7--金币系统优化`、`demo1.0.8--首页二级页面开发+UI`。首页 UI 方向已执行还原清理：旧三主题高保真稿、首页背景图、对齐稿、取用规则说明、导出模板/脚本和旧首页截图均已删除；新版三主题首页完整 UI 设计稿已重新确定并保存。
...
- 首页二级页面：
  ...
  - 三个页面的返回按钮和底部“返回首页”按钮均可回到首页。
  - `scripts/smoke-browser.mjs` 已新增断言：二级页必须由布局根节点渲染，若重新使用整页设计图会失败。
...
- 单元测试：25/25 通过。
- 构建：成功输出到 `dist/`。
- 浏览器 smoke：使用固定 seed 通过，覆盖首页地图、首页背景无图片、个人中心/设置/金币兑换三个二级入口、二级页禁止整页设计图铺底、游戏页、弹框、结果页、体力流程、首通金币、重复通关无金币文案和 0 道具不呼吸状态。
...
- 本轮提交备注：`demo1.0.8--首页二级页面开发+UI`。
- 本轮提交内容：接入个人中心、设置页、金币兑换页三个首页二级入口；三个页面改为真实 HTML/CSS 组件布局，动态数值保留在代码层；新增 smoke 断言防止二级页继续使用整页设计图铺底；同步更新交接文档。
...
5. 二级页后续替换正式图片/icon 时，保持当前 HTML/CSS 结构不变，只替换 `src/assets/UI-ICON/*-page/` 下的独立资源。
